...
public class Raw
{
	...
	public static int readLeInt(DataInput di, byte[] b) throws RZPException{
		try {
			di.readFully(b, 0, 4);
		} catch (IOException e) {
	...
	public static byte bitArrayToByte(int[] bitArray) throws RZPException {
		if (bitArray == null) {
			throw new RZPException("bit array is null, cannot calculate byte from bits");
		}
		
		if (bitArray.length != 8) {
			throw new RZPException("invalid bit array length, cannot calculate byte");
		}
		
		if(!checkBits(bitArray)) {
			throw new RZPException("invalid bits provided, bits contain other values than 0 or 1");
		}
