...
import rmkj.lib.rzp.crypto.engine.ZipCryptoEngine;
...
public class StandardDecrypter implements IDecrypter {
	...
	public StandardDecrypter(FileHeader fileHeader, byte[] headerBytes) throws RZPException{
		if (fileHeader == null) {
			throw new RZPException("one of more of the input parameters were null in StandardDecryptor");
		}
	...
	public int decryptData(byte[] buff, int start, int len) throws RZPException {
		if (start < 0 || len < 0) {
			throw new RZPException("one of the input parameters were null in standard decrpyt data");
		}
	...
	public void init(byte[] headerBytes) throws RZPException {
        ...
        if (fileHeader.getPassword() == null || fileHeader.getPassword().length <= 0) {
        	throw new RZPException("Wrong password!", RZPExceptionConstants.WRONG_PASSWORD);
        }
        ...
        try {
			...
			for (int i = 0; i < InternalRZPConstants.STD_DEC_HDR_SIZE; i++) {
...
//					throw new ZipException("Wrong password!", ZipExceptionConstants.WRONG_PASSWORD);
				...
				if (i+1 != InternalRZPConstants.STD_DEC_HDR_SIZE)
