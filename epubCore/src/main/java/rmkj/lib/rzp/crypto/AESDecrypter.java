...
public class AESDecrypter implements IDecrypter {
	...
	public AESDecrypter(LocalFileHeader localFileHeader, 
			byte[] salt, byte[] passwordVerifier) throws RZPException {
		
		if (localFileHeader == null) {
			throw new RZPException("one of the input parameters is null in AESDecryptor Constructor");
		}
	...
	private void init(byte[] salt, byte[] passwordVerifier) throws RZPException {
		if (localFileHeader == null) {
			throw new RZPException("invalid file header in init method of AESDecryptor");
		}
		
		AESExtraDataRecord aesExtraDataRecord = localFileHeader.getAesExtraDataRecord();
		if (aesExtraDataRecord == null) {
			throw new RZPException("invalid aes extra data record - in init method of AESDecryptor");
		}
		...
		case RZPConstants.AES_STRENGTH_256:
			...
			break;
		default:
			throw new RZPException("invalid aes key strength for file: " + localFileHeader.getFileName());
		}
		
		if (localFileHeader.getPassword() == null || localFileHeader.getPassword().length <= 0) {
			throw new RZPException("empty or null password provided for AES Decryptor");
		}
		...
		if (derivedKey == null || 
			...
			throw new RZPException("invalid derived key");
		}
		...
		if (derivedPasswordVerifier == null) {
			throw new RZPException("invalid derived password verifier for AES");
		}
		
		if (!Arrays.equals(passwordVerifier, derivedPasswordVerifier)) {
			throw new RZPException("Wrong Password for file: " + localFileHeader.getFileName(), RZPExceptionConstants.WRONG_PASSWORD);
		}
	...
	public int decryptData(byte[] buff, int start, int len) throws RZPException {
		
		if (aesEngine == null) {
			throw new RZPException("AES not initialized properly");
		}
	...
	public int decryptData(byte[] buff) throws RZPException {
