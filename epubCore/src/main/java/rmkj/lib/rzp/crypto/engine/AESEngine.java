...
package rmkj.lib.rzp.crypto.engine;
...
public class AESEngine {
    ...
    public AESEngine(byte[] key) throws RZPException {
		init(key);
	}
    
    public void init(byte[] key) throws RZPException {
        workingKey = generateWorkingKey(key);
    }
    
    private int[][] generateWorkingKey(byte[] key) throws RZPException {
        int         kc = key.length / 4;
        int         t;
        
        if (((kc != 4) && (kc != 6) && (kc != 8)) || ((kc * 4) != key.length))
        {
            throw new RZPException("invalid key length (not 128/192/256)");
        }
    ...
    public int processBlock(byte[] in, byte[] out) throws RZPException {
    	return processBlock(in, 0, out, 0);
    }
    
    public int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws RZPException {
        if (workingKey == null)
        {
            throw new RZPException("AES engine not initialised");
        }

        if ((inOff + (32 / 2)) > in.length)
        {
            throw new RZPException("input buffer too short");
        }

        if ((outOff + (32 / 2)) > out.length)
        {
            throw new RZPException("output buffer too short");
        }
        ...
        return InternalRZPConstants.AES_BLOCK_SIZE;
    }
