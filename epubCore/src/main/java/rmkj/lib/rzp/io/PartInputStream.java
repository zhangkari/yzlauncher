...
public class PartInputStream extends BaseInputStream
{
	...
	public PartInputStream(RandomAccessFile raf, long start, long len, UnRZPEngine unzipEngine) {
	    this.raf = raf;
	    ...
	    this.isAESEncryptedFile = unzipEngine.getFileHeader().isEncrypted() && 
			unzipEngine.getFileHeader().getEncryptionMethod() == RZPConstants.ENC_METHOD_AES;
	}
	...
	private void checkAndReadAESMacBytes() throws IOException {
		if (isAESEncryptedFile) {
			if (decrypter != null && decrypter instanceof AESDecrypter) {
				...
				byte[] macBytes = new byte[InternalRZPConstants.AES_AUTH_LENGTH];
				int readLen = -1;
				readLen = raf.read(macBytes);
				if (readLen != InternalRZPConstants.AES_AUTH_LENGTH) {
					if (unzipEngine.getZipModel().isSplitArchive()) {
						raf.close();
						raf = unzipEngine.startNextSplitFile();
						int newlyRead = raf.read(macBytes, readLen, InternalRZPConstants.AES_AUTH_LENGTH - readLen);
						readLen += newlyRead;
					} else {
