...
public class InflaterInputStream extends PartInputStream {
	...
	public InflaterInputStream(RandomAccessFile raf, long start, long len, UnRZPEngine unzipEngine) {
		super(raf, start, len, unzipEngine);
		this.inflater = new Inflater(true);
		this.buff = new byte[InternalRZPConstants.BUFF_SIZE];
		...
		try {
		    ...
		    if (unzipEngine != null) {
		    	if (unzipEngine.getLocalFileHeader().isEncrypted() && 
		    			unzipEngine.getLocalFileHeader().getEncryptionMethod() == RZPConstants.ENC_METHOD_STANDARD) {
		    		s += " - Wrong Password?";
		    	}
		    }
