...
public class RZPParameters implements Cloneable {
	...
	public RZPParameters() {
		compressionMethod = RZPConstants.COMP_DEFLATE;
		encryptFiles = false;
		readHiddenFiles = true;
		encryptionMethod = RZPConstants.ENC_NO_ENCRYPTION;
		aesKeyStrength = -1;
	...
	public void setRootFolderInZip(String rootFolderInZip) {
		if (RZPUtil.isStringNotNullAndNotEmpty(rootFolderInZip)) {
			...
			if (!rootFolderInZip.endsWith("\\") && !rootFolderInZip.endsWith("/")) {
				rootFolderInZip = rootFolderInZip + InternalRZPConstants.FILE_SEPARATOR;
			}
